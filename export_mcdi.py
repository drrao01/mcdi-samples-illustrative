...
def build_messages(final_traj):
    ...
    for m in get(f"/trajectories/{final_traj}")["trajectory_messages"]:
        ...
        if role=="system":
            ...
            # harness scaffolding, not conversation: context-compaction summaries and the
            # stub acknowledgement the runner pairs with them
            if c.lstrip().startswith("**Context Continuation**"): continue
            if c.strip() in ("Got it, thanks!","Got it, thanks."): continue
